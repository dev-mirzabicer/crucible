...
describe("skill tool - agent restriction", () => {
  ...
  it("allows skill when agent matches restriction", async () => {
    // given
    const loadedSkills = [createMockSkill("restricted-skill", { agent: "builder" })]
    const tool = createSkillTool({ skills: loadedSkills })
    const context = { ...mockContext, agent: "builder" }
  ...
  it("throws error when agent does not match restriction", async () => {
    // given
    const loadedSkills = [createMockSkill("builder-only-skill", { agent: "builder" })]
    const tool = createSkillTool({ skills: loadedSkills })
    const context = { ...mockContext, agent: "oracle" }

    // when / #then
    await expect(tool.execute({ name: "builder-only-skill" }, context)).rejects.toThrow(
      'Skill "builder-only-skill" is restricted to agent "builder"'
    )
  })
  ...
  it("throws error when context agent is undefined for restricted skill", async () => {
    ...
    // when / #then
    await expect(tool.execute({ name: "builder-only-skill" }, contextWithoutAgent)).rejects.toThrow(
      'Skill "builder-only-skill" is restricted to agent "builder"'
    )
  })
