...
export async function downloadAstGrep(version: string = DEFAULT_VERSION): Promise<string | null> {
  ...
  if (!platformInfo) {
    log(`[crucible] Unsupported platform for ast-grep: ${platformKey}`)
    return null
  }
  ...
  log(`[crucible] Downloading ast-grep binary...`)
  ...
  try {
    ...
    log(`[crucible] ast-grep binary ready.`)

    return binaryPath
  } catch (err) {
    log(
      `[crucible] Failed to download ast-grep: ${err instanceof Error ? err.message : err}`
    )
    return null
  }
}
