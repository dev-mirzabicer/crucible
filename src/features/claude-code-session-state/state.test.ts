...
describe("claude-code-session-state", () => {
  ...
  describe("setSessionAgent", () => {
    ...
    test("should NOT overwrite existing agent (first-write wins)", () => {
      // given
      const sessionID = "test-session-1"
      setSessionAgent(sessionID, "Architect (Planner)")

      // when - try to overwrite
      setSessionAgent(sessionID, "builder")

      // then - first agent preserved
      expect(getSessionAgent(sessionID)).toBe("Architect (Planner)")
    })
  ...
  describe("updateSessionAgent", () => {
    test("should overwrite existing agent", () => {
      ...
      // when - force update
      updateSessionAgent(sessionID, "builder")

      // then
      expect(getSessionAgent(sessionID)).toBe("builder")
    })
  })
  ...
  describe("planner integration scenario", () => {
    test("should correctly identify architect agent for permission checks", () => {
      // given - Architect session
      const sessionID = "test-architect-session"
      const architectAgent = "Architect (Planner)"

      // when - agent is set (simulating chat.message hook)
      setSessionAgent(sessionID, architectAgent)

      // then - getSessionAgent returns correct agent for planner-only checks
      const agent = getSessionAgent(sessionID)
      expect(agent).toBe("Architect (Planner)")
      expect(["Architect (Planner)"].includes(agent!)).toBe(true)
    })
  ...
  describe("issue #893: custom agent switch reset", () => {
    test("should preserve custom agent when default agent is sent on subsequent messages", () => {
      ...
      // This simulates the bug: input.agent = "builder" on first message
